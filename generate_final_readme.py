"""
VeriTriage: Final README Generator
Merges excellent Copilot content structure with base64 image embedding
This fixes GitHub and VS Code preview image loading issues.

Usage:
    python generate_final_readme.py
"""
...
def embed_image(path):
    """Embed image file as base64 data URI."""
    try:
...
def load_notebook_images():
    """Load images extracted from Jupyter notebooks."""
    try:
...
# ── Load images: prefer notebook images, fallback to PNG files ────────────────
print("Loading images...")
notebook_imgs = load_notebook_images()

if notebook_imgs:
    img_confuse = notebook_imgs.get('03_model_training_img1')
    img_confusion_detail = notebook_imgs.get('03_model_training_img2')
    ...
    img_overview = None
    print(f"  Loaded {sum(1 for x in [img_confuse, img_confusion_detail, img_failure, img_gap, img_shap, img_waterfall] if x)} images from notebook_images.json")
else:
    # Fallback to PNG files
    plots = ROOT / "results/plots"
    img_confuse = embed_image(plots / "03_model_results.png")
    img_confusion_detail = embed_image(plots / "03_confusion_matrices.png")
    ...
    print("  Loaded from PNG files")

def img_tag(b64, alt, width="100%", collapsible=False):
    """Generate HTML img tag with base64 embedding. GitHub and VS Code compatible."""
    if not b64:
...
{img_tag(img_confuse, "Model performance: IR Drop, DRC, Congestion predictions", "100%", collapsible=True)}
...
{img_tag(img_confusion_detail, "Confusion matrices showing prediction accuracy", "100%", collapsible=True)}
...
├── LICENSE                              # MIT License
└── generate_final_readme.py             # Auto-generates README with embedded images
```
...
```bash
# Regenerate README with latest data and embedded images
python generate_final_readme.py
```
...
print(f"\n✓ Final README generated with embedded images!")
print(f"  Location: {output_path}")
print(f"  Size: {file_size_mb:.2f} MB")
print(f"\nImages are now embedded as base64 (viewable everywhere):")
print(f"  ✓ GitHub")
print(f"  ✓ VS Code preview")
print(f"  ✓ Web browsers")
print(f"  ✓ Email clients")
